...
GMSH_API void
gmshModelGetPhysicalGroupsForEntity(const int dim, const int tag, int** physicalTags, size_t* physicalTags_n, int* ierr);
...
GMSH_API int
gmshModelAddPhysicalGroup(const int dim, const int* tags, const size_t tags_n, const int tag, const char* name, int* ierr);
...
GMSH_API void gmshModelGetBoundary(const int*   dimTags,
                                   const size_t dimTags_n,
                                   int**        outDimTags,
                                   size_t*      outDimTags_n,
                                   const int    combined,
                                   const int    oriented,
 ...
 * dimension `dim' - 1. */
GMSH_API void gmshModelGetAdjacencies(
    const int dim, const int tag, int** upward, size_t* upward_n, int** downward, size_t* downward_n, int* ierr);
...
GMSH_API void gmshModelGetBoundingBox(
    const int dim, const int tag, double* xmin, double* ymin, double* zmin, double* xmax, double* ymax, double* zmax, int* ierr);
...
GMSH_API void gmshModelGetValue(const int     dim,
                                ...
                                const size_t  parametricCoord_n,
                                double**      coord,
                                size_t*       coord_n,
                                int*          ierr);
...
GMSH_API void gmshModelGetNormal(
    const int tag, const double* parametricCoord, const size_t parametricCoord_n, double** normals, size_t* normals_n, int* ierr);
...
GMSH_API void gmshModelGetParametrizationBounds(
    const int dim, const int tag, double** min, size_t* min_n, double** max, size_t* max_n, int* ierr);
...
GMSH_API int
gmshModelIsInside(const int dim, const int tag, const double* coord, const size_t coord_n, const int parametric, int* ierr);
...
GMSH_API void gmshModelSetColor(const int*   dimTags,
                                const size_t dimTags_n,
                                const int    r,
                                const int    g,
                                const int    b,
...
GMSH_API void gmshModelMeshPartition(const int     numPart,
                                     const size_t* elementTags,
                                     const size_t  elementTags_n,
                                     const int*    partitions,
                                     const size_t  partitions_n,
                                     int*          ierr);
...
GMSH_API void gmshModelMeshOptimize(
    const char* method, const int force, const int niter, const int* dimTags, const size_t dimTags_n, int* ierr);
...
GMSH_API void gmshModelMeshAffineTransform(
    const double* affineTransform, const size_t affineTransform_n, const int* dimTags, const size_t dimTags_n, int* ierr);
...
GMSH_API void gmshModelMeshGetNodes(size_t**  nodeTags,
                                    size_t*   nodeTags_n,
                                    double**  coord,
                                    size_t*   coord_n,
                                    double**  parametricCoord,
...
GMSH_API void gmshModelMeshGetNodesForPhysicalGroup(
    const int dim, const int tag, size_t** nodeTags, size_t* nodeTags_n, double** coord, size_t* coord_n, int* ierr);
...
GMSH_API void gmshModelMeshAddNodes(const int     dim,
                                    const int     tag,
                                    const size_t* nodeTags,
                                    const size_t  nodeTags_n,
                                    const double* coord,
                                    const size_t  coord_n,
                                    const double* parametricCoord,
                                    const size_t  parametricCoord_n,
 ...
 * e1n2, ..., e1nN, e2n1, ...]. */
GMSH_API void gmshModelMeshGetElements(int**     elementTypes,
...
GMSH_API void gmshModelMeshGetElement(
    const size_t elementTag, int* elementType, size_t** nodeTags, size_t* nodeTags_n, int* dim, int* tag, int* ierr);
 ...
 * often preferable. */
GMSH_API void gmshModelMeshGetLocalCoordinatesInElement(
    const size_t elementTag, const double x, const double y, const double z, double* u, double* v, double* w, int* ierr);
 ...
 * indexed by `task'. */
GMSH_API void gmshModelMeshGetElementQualities(const size_t* elementTags,
                                               const size_t  elementTags_n,
...
GMSH_API void gmshModelMeshAddElements(const int            dim,
                                       const int            tag,
                                       const int*           elementTypes,
                                       const size_t         elementTypes_n,
                                       const size_t* const* elementTags,
                                       const size_t*        elementTags_n,
                                       const size_t         elementTags_nn,
                                       const size_t* const* nodeTags,
                                       const size_t*        nodeTags_n,
                                       const size_t         nodeTags_nn,
                                       int*                 ierr);
...
GMSH_API void gmshModelMeshGetBasisFunctions(const int     elementType,
                                             ...
                                             int*          numOrientations,
                                             const int*    wantedOrientations,
                                             const size_t  wantedOrientations_n,
                                             int*          ierr);
...
GMSH_API void gmshModelMeshPreallocateBasisFunctionsOrientation(
    const int elementType, int** basisFunctionsOrientation, size_t* basisFunctionsOrientation_n, const int tag, int* ierr);
 ...
 * well. */
GMSH_API void gmshModelMeshGetEdges(const size_t* nodeTags,
                                    const size_t  nodeTags_n,
                                    size_t**      edgeTags,
                                    size_t*       edgeTags_n,
...
GMSH_API void gmshModelMeshGetAllFaces(
    const int faceType, size_t** faceTags, size_t* faceTags_n, size_t** faceNodes, size_t* faceNodes_n, int* ierr);

/* Add mesh edges defined by their global unique identifiers `edgeTags' and
 * their nodes `edgeNodes'. */
GMSH_API void gmshModelMeshAddEdges(
    const size_t* edgeTags, const size_t edgeTags_n, const size_t* edgeNodes, const size_t edgeNodes_n, int* ierr);
...
GMSH_API void gmshModelMeshAddFaces(const int     faceType,
                                    const size_t* faceTags,
                                    const size_t  faceTags_n,
                                    const size_t* faceNodes,
                                    const size_t  faceNodes_n,
                                    int*          ierr);
 ...
 * future release. */
GMSH_API void gmshModelMeshGetKeysInformation(const int*    typeKeys,
                                              const size_t  typeKeys_n,
                                              const size_t* entityKeys,
                                              const size_t  entityKeys_n,
                                              const int     elementType,
                                              const char*   functionSpaceType,
                                              int**         infoKeys,
                                              size_t*       infoKeys_n,
 ...
 * and C++ only. */
GMSH_API void
gmshModelMeshPreallocateBarycenters(const int elementType, double** barycenters, size_t* barycenters_n, const int tag, int* ierr);
...
GMSH_API void gmshModelMeshGetGhostElements(
    const int dim, const int tag, size_t** elementTags, size_t* elementTags_n, int** partitions, size_t* partitions_n, int* ierr);
...
GMSH_API void gmshModelMeshSetSizeCallback(
    double (*callback)(int dim, int tag, double x, double y, double z, double lc, void* data), void* callback_data, int* ierr);
...
GMSH_API void
gmshModelMeshSetTransfiniteCurve(const int tag, const int numNodes, const char* meshType, const double coef, int* ierr);
...
GMSH_API void gmshModelMeshSetTransfiniteSurface(
    const int tag, const char* arrangement, const int* cornerTags, const size_t cornerTags_n, int* ierr);
...
GMSH_API void gmshModelMeshSetTransfiniteAutomatic(
    const int* dimTags, const size_t dimTags_n, const double cornerAngle, const int recombine, int* ierr);
...
GMSH_API void
gmshModelMeshEmbed(const int dim, const int* tags, const size_t tags_n, const int inDim, const int inTag, int* ierr);
...
GMSH_API void
gmshModelMeshReorderElements(const int elementType, const int tag, const size_t* ordering, const size_t ordering_n, int* ierr);
...
GMSH_API void gmshModelMeshSetPeriodic(const int     dim,
                                       const int*    tags,
                                       const size_t  tags_n,
                                       const int*    tagsMaster,
                                       const size_t  tagsMaster_n,
                                       const double* affineTransform,
                                       const size_t  affineTransform_n,
                                       int*          ierr);

/* Get master entities `tagsMaster' for the entities of dimension `dim' and
 * tags `tags'. */
GMSH_API void
gmshModelMeshGetPeriodic(const int dim, const int* tags, const size_t tags_n, int** tagMaster, size_t* tagMaster_n, int* ierr);
...
GMSH_API void
gmshModelMeshGetDuplicateNodes(size_t** tags, size_t* tags_n, const int* dimTags, const size_t dimTags_n, int* ierr);
...
GMSH_API void gmshModelMeshAddHomologyRequest(const char*  type,
                                              const int*   domainTags,
                                              const size_t domainTags_n,
                                              const int*   subdomainTags,
                                              const size_t subdomainTags_n,
                                              const int*   dims,
                                              const size_t dims_n,
                                              int*         ierr);
...
GMSH_API void
gmshModelMeshFieldSetNumbers(const int tag, const char* option, const double* value, const size_t value_n, int* ierr);
...
GMSH_API int
gmshModelGeoAddPoint(const double x, const double y, const double z, const double meshSize, const int tag, int* ierr);
...
GMSH_API int
gmshModelGeoAddCompoundSpline(const int* curveTags, const size_t curveTags_n, const int numIntervals, const int tag, int* ierr);
 ...
 * tag of the b-spline. */
GMSH_API int
gmshModelGeoAddCompoundBSpline(const int* curveTags, const size_t curveTags_n, const int numIntervals, const int tag, int* ierr);
...
GMSH_API int
gmshModelGeoAddCurveLoop(const int* curveTags, const size_t curveTags_n, const int tag, const int reorient, int* ierr);
...
GMSH_API int
gmshModelGeoAddSurfaceFilling(const int* wireTags, const size_t wireTags_n, const int tag, const int sphereCenterTag, int* ierr);
...
GMSH_API int gmshModelGeoAddGeometry(const char*        geometry,
                                     ...
                                     const size_t       numbers_n,
                                     const char* const* strings,
                                     const size_t       strings_n,
                                     const int          tag,
                                     int*               ierr);
...
GMSH_API int gmshModelGeoAddPointOnGeometry(
    const int geometryTag, const double x, const double y, const double z, const double meshSize, const int tag, int* ierr);
 ...
 * layers. */
GMSH_API void gmshModelGeoExtrude(const int*    dimTags,
                                  const size_t  dimTags_n,
                                  const double  dx,
                                  const double  dy,
                                  const double  dz,
                                  ...
                                  const size_t  numElements_n,
                                  const double* heights,
                                  const size_t  heights_n,
                                  const int     recombine,
                                  int*          ierr);
 ...
 * layers, normalized to 1. If `recombine' is set, recombine the mesh in the
 * layers. */
GMSH_API void gmshModelGeoRevolve(const int*    dimTags,
                                  const size_t  dimTags_n,
                                  const double  x,
                                  const double  y,
                                  const double  z,
...
GMSH_API void gmshModelGeoTwist(const int*    dimTags,
                                const size_t  dimTags_n,
                                const double  x,
                                const double  y,
                                const double  z,
...
GMSH_API void gmshModelGeoExtrudeBoundaryLayer(const int*    dimTags,
                                               const size_t  dimTags_n,
                                               int**         outDimTags,
                                               size_t*       outDimTags_n,
                                               const int*    numElements,
 ...
 * (`dx', `dy', `dz'). */
GMSH_API void
gmshModelGeoTranslate(const int* dimTags, const size_t dimTags_n, const double dx, const double dy, const double dz, int* ierr);
...
GMSH_API void gmshModelGeoRotate(const int*   dimTags,
                                 const size_t dimTags_n,
                                 const double x,
                                 const double y,
                                 const double z,
...
GMSH_API void gmshModelGeoDilate(const int*   dimTags,
                                 const size_t dimTags_n,
                                 const double x,
                                 const double y,
                                 const double z,
...
GMSH_API void gmshModelGeoMirror(
    const int* dimTags, const size_t dimTags_n, const double a, const double b, const double c, const double d, int* ierr);

/* Mirror the entities `dimTag' in the built-in CAD representation, with
 * respect to the plane of equation `a' * x + `b' * y + `c' * z + `d' = 0.
 * (This is a synonym for `mirror', which will be deprecated in a future
 * release.) */
GMSH_API void gmshModelGeoSymmetrize(
    const int* dimTags, const size_t dimTags_n, const double a, const double b, const double c, const double d, int* ierr);
 ...
 * created curve(s). */
GMSH_API void gmshModelGeoSplitCurve(
    const int tag, const int* pointTags, const size_t pointTags_n, int** curveTags, size_t* curveTags_n, int* ierr);
...
GMSH_API int
gmshModelGeoAddPhysicalGroup(const int dim, const int* tags, const size_t tags_n, const int tag, const char* name, int* ierr);
...
GMSH_API void
gmshModelGeoMeshSetTransfiniteCurve(const int tag, const int nPoints, const char* meshType, const double coef, int* ierr);
...
GMSH_API int
gmshModelOccAddPoint(const double x, const double y, const double z, const double meshSize, const int tag, int* ierr);
...
GMSH_API int gmshModelOccAddCircle(const double  x,
                                   ...
                                   const double  angle2,
                                   const double* zAxis,
                                   const size_t  zAxis_n,
                                   const double* xAxis,
                                   const size_t  xAxis_n,
                                   int*          ierr);
...
GMSH_API int gmshModelOccAddEllipseArc(
    const int startTag, const int centerTag, const int majorTag, const int endTag, const int tag, int* ierr);
...
GMSH_API int gmshModelOccAddBSpline(const int*    pointTags,
                                    const size_t  pointTags_n,
                                    const int     tag,
                                    const int     degree,
                                    const double* weights,
                                    const size_t  weights_n,
                                    const double* knots,
                                    const size_t  knots_n,
                                    const int*    multiplicities,
                                    const size_t  multiplicities_n,
                                    int*          ierr);
...
GMSH_API int gmshModelOccAddBSplineSurface(const int*    pointTags,
                                           const size_t  pointTags_n,
                                           ...
                                           const size_t  weights_n,
                                           const double* knotsU,
                                           const size_t  knotsU_n,
                                           const double* knotsV,
                                           const size_t  knotsV_n,
                                           const int*    multiplicitiesU,
                                           const size_t  multiplicitiesU_n,
                                           const int*    multiplicitiesV,
                                           const size_t  multiplicitiesV_n,
                                           const int*    wireTags,
                                           const size_t  wireTags_n,
                                           const int     wire3D,
                                           int*          ierr);
...
GMSH_API int gmshModelOccAddBezierSurface(const int*   pointTags,
                                          const size_t pointTags_n,
                                          const int    numPointsU,
                                          const int    tag,
                                          const int*   wireTags,
...
GMSH_API int gmshModelOccAddTrimmedSurface(
    const int surfaceTag, const int* wireTags, const size_t wireTags_n, const int wire3D, const int tag, int* ierr);
...
GMSH_API int
gmshModelOccAddSurfaceLoop(const int* surfaceTags, const size_t surfaceTags_n, const int tag, const int sewing, int* ierr);
...
GMSH_API int gmshModelOccAddBox(
    const double x, const double y, const double z, const double dx, const double dy, const double dz, const int tag, int* ierr);
...
GMSH_API void gmshModelOccAddThruSections(const int*   wireTags,
                                          const size_t wireTags_n,
                                          int**        outDimTags,
                                          size_t*      outDimTags_n,
                                          const int    tag,
...
GMSH_API void gmshModelOccAddThickSolid(const int    volumeTag,
                                        const int*   excludeSurfaceTags,
                                        const size_t excludeSurfaceTags_n,
                                        const double offset,
                                        int**        outDimTags,
                                        size_t*      outDimTags_n,
                                        const int    tag,
 ...
 * layers, normalized to 1. If `recombine' is set, recombine the mesh in the
 * layers. */
GMSH_API void gmshModelOccExtrude(const int*    dimTags,
                                  const size_t  dimTags_n,
                                  const double  dx,
                                  const double  dy,
                                  const double  dz,
 ...
 * mesh in the layers. */
GMSH_API void gmshModelOccRevolve(const int*    dimTags,
                                  const size_t  dimTags_n,
                                  const double  x,
                                  const double  y,
                                  const double  z,
...
GMSH_API void gmshModelOccAddPipe(const int*   dimTags,
                                  const size_t dimTags_n,
                                  const int    wireTag,
                                  int**        outDimTags,
                                  size_t*      outDimTags_n,
 ...
 * `removeVolume' is set. */
GMSH_API void gmshModelOccFillet(const int*    volumeTags,
                                 const size_t  volumeTags_n,
                                 const int*    curveTags,
                                 const size_t  curveTags_n,
                                 const double* radii,
                                 const size_t  radii_n,
                                 int**         outDimTags,
                                 size_t*       outDimTags_n,
 ...
 * `removeVolume' is set. */
GMSH_API void gmshModelOccChamfer(const int*    volumeTags,
                                  const size_t  volumeTags_n,
                                  const int*    curveTags,
                                  const size_t  curveTags_n,
                                  const int*    surfaceTags,
                                  const size_t  surfaceTags_n,
                                  const double* distances,
                                  const size_t  distances_n,
                                  int**         outDimTags,
                                  size_t*       outDimTags_n,
                                  const int     removeVolume,
                                  int*          ierr);
 ...
 * `removeTool' is set. */
GMSH_API void gmshModelOccFuse(const int*   objectDimTags,
                               const size_t objectDimTags_n,
                               const int*   toolDimTags,
                               const size_t toolDimTags_n,
                               int**        outDimTags,
                               size_t*      outDimTags_n,
                               int***       outDimTagsMap,
...
GMSH_API void gmshModelOccIntersect(const int*   objectDimTags,
                                    const size_t objectDimTags_n,
                                    const int*   toolDimTags,
                                    const size_t toolDimTags_n,
                                    int**        outDimTags,
 ...
 * entity). Remove the object if `removeObject' is set. Remove the tool if
 * `removeTool' is set. */
GMSH_API void gmshModelOccCut(const int*   objectDimTags,
                              const size_t objectDimTags_n,
                              const int*   toolDimTags,
                              const size_t toolDimTags_n,
                              int**        outDimTags,
...
GMSH_API void gmshModelOccFragment(const int*   objectDimTags,
                                   const size_t objectDimTags_n,
                                   const int*   toolDimTags,
                                   const size_t toolDimTags_n,
                                   int**        outDimTags,
 ...
 * along (`dx', `dy', `dz'). */
GMSH_API void
gmshModelOccTranslate(const int* dimTags, const size_t dimTags_n, const double dx, const double dy, const double dz, int* ierr);
...
GMSH_API void gmshModelOccRotate(const int*   dimTags,
                                 const size_t dimTags_n,
                                 const double x,
                                 const double y,
                                 const double z,
...
GMSH_API void gmshModelOccDilate(const int*   dimTags,
                                 const size_t dimTags_n,
                                 const double x,
                                 const double y,
                                 const double z,
 ...
 * respect to the plane of equation `a' * x + `b' * y + `c' * z + `d' = 0. */
GMSH_API void gmshModelOccMirror(
    const int* dimTags, const size_t dimTags_n, const double a, const double b, const double c, const double d, int* ierr);

/* Mirror the entities `dimTags' in the OpenCASCADE CAD representation, with
 * respect to the plane of equation `a' * x + `b' * y + `c' * z + `d' = 0.
 * (This is a deprecated synonym for `mirror'.) */
GMSH_API void gmshModelOccSymmetrize(
    const int* dimTags, const size_t dimTags_n, const double a, const double b, const double c, const double d, int* ierr);
...
GMSH_API void gmshModelOccAffineTransform(
    const int* dimTags, const size_t dimTags_n, const double* affineTransform, const size_t affineTransform_n, int* ierr);
...
GMSH_API void gmshModelOccHealShapes(int**        outDimTags,
                                     size_t*      outDimTags_n,
                                     const int*   dimTags,
                                     const size_t dimTags_n,
...
GMSH_API void gmshModelOccImportShapes(
    const char* fileName, int** outDimTags, size_t* outDimTags_n, const int highestDimOnly, const char* format, int* ierr);
...
GMSH_API void gmshModelOccImportShapesNativePointer(
    const void* shape, int** outDimTags, size_t* outDimTags_n, const int highestDimOnly, int* ierr);
...
GMSH_API void
gmshViewAddListData(const int tag, const char* dataType, const int numEle, const double* data, const size_t data_n, int* ierr);
...
GMSH_API void gmshViewSetInterpolationMatrices(const int     tag,
                                               ...
                                               const size_t  exp_n,
                                               const int     dGeo,
                                               const double* coefGeo,
                                               const size_t  coefGeo_n,
                                               const double* expGeo,
                                               const size_t  expGeo_n,
                                               int*          ierr);
...
GMSH_API void gmshViewProbe(const int     tag,
                            ...
                            const size_t  xElemCoord_n,
                            const double* yElemCoord,
                            const size_t  yElemCoord_n,
                            const double* zElemCoord,
                            const size_t  zElemCoord_n,
                            const int     dim,
                            int*          ierr);
 ...
 * between 0 and 255. */
GMSH_API void
gmshViewOptionSetColor(const int tag, const char* name, const int r, const int g, const int b, const int a, int* ierr);
