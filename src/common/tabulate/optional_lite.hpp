 ...
 * @brief Third-party single-header backport of std::optional (optional-lite by
 *        Martin Moene). Provides nonstd::optional as a drop-in replacement for
 *        pre-C++17 toolchains; when a standard <optional> is available it simply
 *        re-exports the std types into namespace nonstd. Included by tabulate to
 *        support compilers without C++17. Vendored as-is; not documented in detail.
 */
